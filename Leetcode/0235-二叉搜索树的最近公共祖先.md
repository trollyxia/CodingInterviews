...
<img src="images/binarysearchtree_improved.png" style="zoom:80%;" />
...
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        ...
        if (larger->val >= root->val && smaller->val <= root->val) return root;
        if (larger->val < root->val) return lowestCommonAncestor(root->left, p, q);
        if (smaller->val > root->val) return lowestCommonAncestor(root->right, p, q);
        return nullptr;
    }
};
```
